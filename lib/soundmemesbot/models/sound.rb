class Sound < Sequel::Model(:sounds)
  ...
  def self.fetch_by_query(query, order_by_choices_count: true)
    ...
    q = Sound.select_all(:sounds).where("tag_ids @> ARRAY[(
      SELECT
        id
      FROM
        tags
      WHERE
        content ILIKE '%#{ query }%' ESCAPE '\'
    )]").or(Sequel.ilike(:title, "%#{ query }%"))

    q = q.association_join(:choices).group_by(:sounds__id).order{ count(:choices__id).desc } if order_by_choices_count
  ...
  def self.fetch_trending(time: 24 * 60 * 60)
    q = Sound.select_all(:sounds).association_join(:choices).where{ choices__created_at >= Time.at(Time.now.to_i - time) }.group_by(:sounds__id).order{ count(:choices__id).desc }
